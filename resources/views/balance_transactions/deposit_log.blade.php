	...
	<div class="col-md-12">
		...
		<tbody>
			...
			<tr>
				...
				<td align="right">{{ ($d->jenis_transaksi === 'Ambil Saldo' ? '-' : '') . number_format($d->deposit + $d->unique_code, 0, ',', '.') }}</td>
				...
				<td align="right">{{ ($d->jenis_transaksi === 'Ambil Saldo' ? '-' : '') .$d->jumlah_disetujui_rp }}</td>
				<td>{{ $d->status }}</td>
